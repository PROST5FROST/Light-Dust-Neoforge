...
public class HandheldLightManager {
    ...
    public record LightData(int radius, float[] color) {
    }
    ...
    private static void extractStacksFrom(Object obj, int depth, java.util.List<ItemStack> found) {
        ...
        switch (obj) {
            ...
            case java.util.Optional optional -> {
                optional.ifPresent(o -> extractStacksFrom(o, depth + 1, found));
                return;
            }
            default -> {
            }
    ...
    public static LightData getHeldLight(Player player) {
        ...
        // Prevents iterating inventory and doing reflection thousands of times a tick
        ...
        if (hasCurios) {
            try {
                ...
                if (opt.isPresent()) {
                    IItemHandler handler = (IItemHandler) opt.get();
